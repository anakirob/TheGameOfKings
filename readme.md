       ___________ ________  __________ __________
      r.          T        7T.         7.     7.  7
      L_          .        ||.         |.         |
        7 .     r|.   _____j|.    .    |.        -+
        | .     ||.   L_    7.   L.    |.         |
        | ...   ||....      |...       |...    .. |
        L_______jl__________j__________j______L___j

                  THE  GAME  OF  KINGS
                    "it's NOT chess"

                        STARRING

    Lord        moves one square any direction
            can summon a new piece to an adjacent square

    Queen       moves in any direction
            can explode to destroy all enemy units except the lord

    Castle      moves on the vertical and horizontal
            can teelport to swap places with the Lord

    Horsey      moves four squares in an L shape
            can go bezerk and kill ALL surrounding units

    Cleric      moves in the diagonal directions
            can fire a projectile to hit any enemy except the lord

    Peasant     moves one square horizontally or vertically
            attacks one square diagonally
            can charge to move two squares killing any obstacle

                        HOW TO PLAY
            the Lords stand in opposite corners of an 8x8 board
            for victory you must subdue the enemy lord to avoid
            defeat you must not allow your Lord to be subdued

            click on a piece to initiate movement or double click
            to perform a special move which can be done only once
            with the notable exception of the Lords summon ability

            when summoning use the mouse wheel or click the icon
            for the desired character all but the peasants have a
            limited number of available units replenished when a
            peasant makes it to the other side of the board

                        QUICK START

https://the-game-of-kings.onrender.com/
